...
def create_detection_timeline_chart():
    ...
    fig.update_layout(
        ...
        margin=dict(l=20, r=20, t=75, b=130)
    )
...
def create_detection_rate_gauge():
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            'axis': {'range': [None, 35], 'tickwidth': 1, 'tickcolor': "white"},
            'bar': {'color': "#4ECDC4"},
            ...
            'steps': [
                ...
                {'range': [20, 25], 'color': 'rgba(78, 205, 196, 0.3)'},
                {'range': [25, 35], 'color': 'rgba(78, 205, 196, 0.3)'}
            ],
            'threshold': {
...
while ret:
    ...
    for license_plate in license_plates.boxes.data.tolist():
        ...
        if car_id != -1:
            ...
            if license_plate_text is not None:
                ...
                (text_width, text_height), _ = cv2.getTextSize(license_plate_text, cv2.FONT_HERSHEY_SIMPLEX, 1.1, 2)
                ...
                cv2.putText(frame, license_plate_text, (text_x_centered, text_y_centered), cv2.FONT_HERSHEY_SIMPLEX, 1.1, (0, 0, 0), 2, cv2.LINE_AA)
                ...
                cv2.rectangle(frame, (lbl_x - 3, lbl_y - lbl_h - 3), (lbl_x + lbl_w + 3, lbl_y + 3), (255, 255, 255), -1)
    ...
    now = datetime.now()
    cv2.putText(frame, f'Frame: {frame_nmr}', (15, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)
    cv2.putText(frame, f'Date: {now.strftime("%Y-%m-%d")}', (15, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)
    cv2.putText(frame, f'Time: {now.strftime("%H:%M:%S")}', (15, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)
    ...
    for row_indx in range(len(df_frame)):
        try:
            ...
            if plate_text != '0' and plate_text != 'N/A':
                (text_width, text_height), _ = cv2.getTextSize(plate_text, cv2.FONT_HERSHEY_SIMPLEX, 1.1, 2)
                ...
                cv2.putText(frame, plate_text, (text_x_centered, text_y_centered), cv2.FONT_HERSHEY_SIMPLEX, 1.1, (0, 0, 0), 2, cv2.LINE_AA)
